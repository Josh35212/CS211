"""
Credit: Chatgpt ifgen function, comparisons 
Expressions
...
class Read(Expr):
    ...
    def gen(self, context: Context, target: str):
        """Read from memory-mapped address 510"""
...
class NE(Comparison):
    """left != right"""
    def __init__(self, left: Expr, right: Expr):
        super().__init__(left, right, "!=", "PM", "Z")  # jump if P or M (not zero) for true, Z for false
...
class GT(Comparison):
    """left > right"""
    def __init__(self, left: Expr, right: Expr):
        super().__init__(left, right, ">", "P", "ZM")  # jump if positive (P) for true, zero or negative (ZM) for false
...
class GE(Comparison):
    """left >= right"""
    def __init__(self, left: Expr, right: Expr):
        super().__init__(left, right, ">=", "PZ", "M")  # jump if positive or zero (PZ) for true, negative (M) for false
...
class LT(Comparison):
    """left < right"""
    def __init__(self, left: Expr, right: Expr):
        super().__init__(left, right, "<", "M", "PZ")  # jump if negative (M) for true, zero or positive (PZ) for false
...
class LE(Comparison):
    """left <= right"""
    def __init__(self, left: Expr, right: Expr):
        super().__init__(left, right, "<=", "ZM", "P")  # jump if zero or negative (ZM) for true, positive (P) for false
...
class If(Control):
    ...
    def gen(self, context: Context, target: str):
        """Generate code for if/then/else/fi"""
        # Get fresh labels
        else_label = context.new_label("else")
        fi_label = context.new_label("fi")
        # Test condition; jump to else if false
