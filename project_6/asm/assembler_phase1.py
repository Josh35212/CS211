...
Josh Gilliam
Info: CS211 3/3/26
Credit: Chatgpt helped with transform()
...
class AsmSrcKind(Enum):
    ...
    # location in place of its source and offset parts.
    MEMOP = auto()
    JUMP = auto()
...
\s*
   ...
   \s*
    ...
    \s+
    (?P<target>    r[0-9]+),            # Target register
    (?P<labelref>  [a-zA-Z]\w*)         # Memory label reference 
   # Optional comment follows # or ; 
   (
     \s*
     (?P<comment>[\#;].*)
...
\s*
   ...
   \s*
    ...
    (/ (?P<predicate> [A-Z]+) )?   # Predicate (optional)
    \s+
    (?P<labelref>  [a-zA-Z]\w*)         # Target label reference 
   # Optional comment follows # or ; 
   (
     \s*
     (?P<comment>[\#;].*)
    ...
    for lnum in range(len(lines)):
        ...
        try: 
            ...
            if fields.get("label"):
                ...
                log.debug(f"Label {label} at address {address}")
            # Handle comments (don't take space in memory)
            if fields["kind"] != AsmSrcKind.COMMENT:
                address += 1
        except  Exception as e:
