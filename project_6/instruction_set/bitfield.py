...
Josh Gilliam
Info: CS211 2/24/26
Credit: ChatGPT helped with sign_extend
...
class BitField(object):
    ...
    def insert(self, value: int, word: int) -> int: 
        ...
        # Originally failed Test_Signed_Insert
        return ((value & self.mask) << self.from_bit) | word    # Mask to make negative numbers work 
    
    def extract_signed(self, word: int) -> int:
        """Extract bits in bitfield as a signed integer."""
        unsigned = self.extract(word)
        return sign_extend(unsigned, self.width)         # Use the helper function sign_extend
